 ...
 *   subset_cnt Int -> 3 --  vectors in dataset will be split in subset_cnt parts
 *                       --  and k-means will be run for each part returning centroids
 *                       --  equal to cluster count for each subset
 ...
 * returns:
 *   3d array -> [ [ [ [f32 x subset_dim ] ] x cluster_cnt ] x subset_cnt ]
 */
...
Datum       create_pq_codebook(PG_FUNCTION_ARGS)
{
    ...
    uint32                subset_cnt = PG_GETARG_UINT32(3);
    ...
    if(dataset_dim < subset_cnt) {
        elog(ERROR, "Dataset dimension can not be smaller than subset count");
    }

    // TODO:: this check can be removed as soon as we resolve return type issue from this function
    if(dataset_dim % subset_cnt != 0) {
        elog(ERROR, "Dataset dimensions should be divisible by subset count");
    }

    elog(INFO, "Starting k-means over dataset with (subsets=%d, clusters=%d)", subset_cnt, cluster_cnt);
    codebooks = product_quantization(cluster_cnt, subset_cnt, dataset, dataset_size, dataset_dim, distance_metric, 200);
    ...
    // Dimensions for parent array the subset count
    codebook_dims[ 0 ] = subset_cnt;
    ...
    // TODO:: curerntly we can not have subsets of different
    // Dimensions because of this
    // Dimensions for each centroid array the subset dimension
    ...
    array = construct_md_array(
        codebook_datums, NULL, 3, codebook_dims, codebook_lbs, FLOAT4OID, sizeof(float4), true, 'i');
