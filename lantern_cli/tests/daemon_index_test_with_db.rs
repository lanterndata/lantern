...
#[tokio::test]
async fn test_daemon_external_index_wrong_ops() {
    let (new_connection_uri, mut new_db_client) =
        setup_test("test_daemon_external_index_wrong_ops")
            .await
            .unwrap();
    new_db_client
        .batch_execute(&format!(
    ...
    INSERT INTO _lantern_extras_internal.external_index_jobs ("id", "table", "column", "operator", "index", efc, ef, m)
    VALUES (2, '{CLIENT_TABLE_NAME}', 'title_embedding', 'cos', 'test_idx1', 32, 32, 10);
     "#
        ))
        .await
        .unwrap();
    ...
    wait_for_completion(
        &mut new_db_client,
        &format!(
            "SELECT COUNT(*)=1 FROM _lantern_extras_internal.external_index_jobs WHERE failed_at IS NOT NULL"
        ),
        30,
    )
    .await
