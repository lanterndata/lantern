...
function setup_locale_and_install_packages() {
  ...
  apt update -y
  apt install -y --no-install-recommends locales lsb-core lsb-release automake cmake wget git build-essential ca-certificates zlib1g-dev pkg-config dpkg-dev libreadline-dev curl gnupg libssl-dev jq clang-format clang llvm lcov
...
function install_platform_specific_dependencies() {
  ...
  pushd /tmp
    ...
    if [[ "$INSTALL_CLI" = "1" ]]
    then
      ORT_STRATEGY=system cargo install --sudo --debug --bin lantern-cli --root /tmp --path /tmp/lantern/lantern_cli
    fi
