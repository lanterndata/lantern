...
CREATE FUNCTION ldb_generic_dist (integer[], integer[])
    RETURNS real
    AS 'MODULE_PATHNAME'
    LANGUAGE C
    IMMUTABLE STRICT PARALLEL SAFE;

CREATE FUNCTION l2sq_dist (real[], real[])
    RETURNS real
    AS 'MODULE_PATHNAME'
    LANGUAGE C
    IMMUTABLE STRICT PARALLEL SAFE;

CREATE FUNCTION cos_dist (real[], real[])
    RETURNS real
    AS 'MODULE_PATHNAME'
    LANGUAGE C
    IMMUTABLE STRICT PARALLEL SAFE;

CREATE FUNCTION hamming_dist (integer[], integer[])
    RETURNS integer
    AS 'MODULE_PATHNAME'
    LANGUAGE C
    IMMUTABLE STRICT PARALLEL SAFE;
...
-- operator classes
CREATE OPERATOR CLASS dist_l2sq_ops DEFAULT FOR TYPE real[]
    USING hnsw AS
    OPERATOR 1 <-> (real[], real[]) FOR
ORDER BY
    float_ops,
    FUNCTION 1 l2sq_dist (real[], real[]
);

CREATE OPERATOR CLASS dist_cos_ops FOR TYPE real[]
    USING hnsw AS
    OPERATOR 1 <-> (real[], real[]) FOR
ORDER BY
    float_ops,
...
CREATE OPERATOR CLASS dist_hamming_ops FOR TYPE integer[]
    USING hnsw AS
    OPERATOR 1 <-> (integer[], integer[]) FOR
ORDER BY
    float_ops,
    ...
    SELECT
        EXISTS (
            ...
            FROM
                pg_type
            WHERE
                typname = 'vector') INTO type_exists;
    IF type_exists THEN
        -- The type exists
        -- taken from pgvector so our index can work with pgvector types
        CREATE FUNCTION vector_l2sq_dist (vector, vector )
            RETURNS float8 AS 'MODULE_PATHNAME'
            LANGUAGE C
            IMMUTABLE STRICT PARALLEL SAFE;
    CREATE OPERATOR CLASS dist_vec_l2sq_ops DEFAULT FOR TYPE vector
        USING hnsw AS
        OPERATOR 1 <-> ( vector, vector) FOR
    ORDER BY
        float_ops,
