...
FROM (
    SELECT
        id,
        ROUND(vector_l2sq_dist (vector, '[0,1,0]')::numeric, 2) AS dist
    FROM
        small_world
    ORDER BY
        vector <-> '[0,1,0]'
...
SELECT
    id,
    ROUND(vector_l2sq_dist (v, :'v42')::numeric, 2)
FROM
    sift_base1k
ORDER BY
    v <-> :'v42'
...
-- another insert test
CREATE TABLE new_small_world AS
SELECT
    *
FROM
    small_world;
...
INSERT INTO new_small_world (id, vector)
    VALUES ('000', '[0,0,0]'),
    ('001', '[0,0,1]'),
    ('010', '[0,1,0]'),
    ('011', '[0,1,1]'),
...
-- index scan
SELECT
    '[0,0,0]'::vector AS v42 \gset

EXPLAIN
SELECT
    id,
    ROUND(vector_l2sq_dist (vector, :'v42')::numeric, 2)
FROM
    new_small_world
ORDER BY
    vector <-> :'v42'
