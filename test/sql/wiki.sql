...
ALTER TABLE ONLY tsv_data
    ALTER COLUMN id SET DEFAULT nextval('tsv_data_id_seq'::regclass);
...
WITH t AS (
    SELECT
        id,
        page_title,
        vector_l2sq_dist (context_page_description_ai, (
                SELECT
                    context_page_description_ai
                FROM tsv_data
                WHERE
                    id = 81386)) AS dist
FROM
...
FROM
    tsv_data
WHERE
    context_page_description_ai IS NOT NULL
LIMIT 444;
