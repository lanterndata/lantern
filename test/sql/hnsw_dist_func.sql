...
INSERT INTO small_world_l2_vec (id, vector)
    VALUES ('000', '[0,0,0]'),
    ('001', '[0,0,1]'),
    ('010', '[0,1,0]'),
    ('011', '[0,1,1]'),
...
INSERT INTO small_world_cos (id, vector)
    VALUES ('000', '{0,0,0}'),
    ('001', '{0,0,1}'),
    ('010', '{0,1,0}'),
    ('011', '{0,1,1}'),
...
INSERT INTO small_world_ham (id, vector)
    VALUES ('000', '{0,0,0}'),
    ('001', '{0,0,1}'),
    ('010', '{0,1,0}'),
    ('011', '{0,1,1}'),
...
FROM (
    SELECT
        id,
        ROUND(l2sq_dist (vector, '[0,1,0]'::vector)::numeric, 2) AS dist
    FROM
        small_world_l2_vec
    ORDER BY
        vector <-> '[0,1,0]'::vector
...
FROM (
    SELECT
        id,
        ROUND(cos_dist (vector, ARRAY[0, 1, 0])::numeric, 2) AS dist
    FROM
        small_world_cos
    ORDER BY
        vector <-> ARRAY[0, 1, 0]
...
FROM (
    SELECT
        id,
        ROUND(hamming_dist (vector, ARRAY[0, 1, 0])::numeric, 2) AS dist
    FROM
        small_world_ham
    ORDER BY
        vector <-> ARRAY[0, 1, 0]
...
FROM (
    SELECT
        id,
        ROUND((vector <-> ARRAY[0, 1, 0])::numeric, 2) AS dist
    FROM
        small_world_ham
    ORDER BY
        vector <-> ARRAY[0, 1, 0]
